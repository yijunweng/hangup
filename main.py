...
# 开启脚本
def start(region, seconds):
    time.sleep(2) # sleep5秒等待控制权释放

    for item in region:
        reset()
        openList()
        print(f'当前地下城：{ item.name }, 进度{ item.index }/{ len(region) }')
        # 选择大陆
        imitate_click(item.position1[0], item.position1[1])
        if item.index > 10: simulate_mouse_drag()  # 10以后的副本要执行拖动事件
        # 选择副本
        imitate_click(item.position2[0], item.position2[1])
        enter()
        time.sleep(seconds)
        print(f'地下城{ item.name }已完成, 进度{ item.index + 1 }/{len(region)}')
        clearBag()
